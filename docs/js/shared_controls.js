...
$(".set-selector").change(function () {
	...
	if ($(this).hasClass('opposing')) {
		...
		if (nextTrainer) {
			...
			const idx = parseInt(trainerId);
			nextTrainerId = idx + 1;
			previousTrainerId = idx - 1;
		}
...
$(".gen").change(function () {
	...
	setdex = SETDEX[gen];
	// regenerate trainer name list for current generation
	TR_NAMES = get_trainer_names();
	randdex = RANDDEX[gen];
	...
	// choose Cooperhawk's Starly as the initial set if it exists
	var coopDefault = "Starly (COOPERHAWK)";
	if (setdex["Starly"] && setdex["Starly"]["COOPERHAWK"]) {
		$(".set-selector").val(coopDefault);
	} else {
...
function get_trainer_names() {
	// use whichever setdex is currently active (fallback to SS if undefined)
	var all_poks = typeof setdex !== 'undefined' ? setdex : SETDEX_SS;
	var trainer_names = []
...
function selectTrainer(value) {
	...
	// use current setdex so that navigation works in the active generation
	var all_poks = typeof setdex !== 'undefined' ? setdex : SETDEX_SS;
	for (const [pok_name, poks] of Object.entries(all_poks)) {
		var pok_tr_names = Object.keys(poks)
		for (i in pok_tr_names) {
...
$(document).ready(function () {
	var params = new URLSearchParams(window.location.search);
	// default to Gen 9 (SV) when no query string is provided
	...
	// prefer Cooperhawk's Starly on fresh load
	var coopDefault = "Starly (COOPERHAWK)";
	if (setdex["Starly"] && setdex["Starly"]["COOPERHAWK"]) {
		$(".set-selector").val(coopDefault);
	} else {
